...
function addItem() {
  ...
  if (todoItem == "") {
    document.getElementById("error").innerHTML = "Cannot add empty task!!!"
  } else {
    if (flagComplete == 1) {
...
function displayTodo() {
  ...
  todo.forEach((element) => {
    str +=
      ...
      i +
      " onclick='completedTask(this.id)'><label>" +
      element +
      "</label><input type='text'><button id='" +
      i +
      "' onclick='updateTodo(this.id)'>Edit</button><button id='delete" +
      i +
      "' onclick='deleteItem(this.id)'>Delete</button></li>";
    i++;
  });
...
function completedTask(id) {
  ...
  completed.forEach((element) => {
    data +=
      "<li><input type='checkbox' id=c" +
      i +
      " onclick='completedTaskRevert(this.id)' value=true><label>" +
      element +
      "</label><input type='text'><button id='" +
      i +
      "' onclick='updateTodoFromCompleted(this.id)'>Edit</button><button id='deletec" +
      i +
      "' onclick='deleteItemFromCompleted(this.id)'>Delete</button></li>";
    i++;
  });
...
function displayCompleted() {
  ...
  completed.forEach((element) => {
    str +=
      "<li><input type='checkbox' id=c" +
      i +
      " onclick='completedTaskRevert(this.id)'><label>" +
      element +
      "</label><input type='text'><button id='" +
      i +
      "' onclick='updateTodoFromCompleted(this.id)'>Edit</button><button id='deletec" +
